...
const (
	...
	didWebToBeResolved     DIDWeb = "did:web:demo.vc-sdk.com"
...
func TestDIDWebResolveDocBytes(t *testing.T) {
	t.Run("Happy Path", func(tt *testing.T) {
		gock.New("https://demo.vc-sdk.com").
			Get("/.well-known/did.json").
			Reply(200).
			BodyString(`{"didDocument": {"id": "did:web:demo.vc-sdk.com"}}`)
		defer gock.Off()

		docBytes, _, err := didWebToBeResolved.resolveDocBytes(context.Background())
		assert.NoError(tt, err)
		assert.Contains(tt, string(docBytes), "did:web:demo.vc-sdk.com")
	})
...
func TestDIDWebResolve(t *testing.T) {
	...
	t.Run("Unhappy Path - Mismatched DID", func(tt *testing.T) {
		...
		assert.Error(tt, err)
		assert.Contains(tt, err.Error(), "doc.id<did:web:demo.vc-sdk.com> does not match did:web value<did:web:doesnotexist.com>")
	})
...
func TestDIDWebValidate(t *testing.T) {
	t.Run("Happy Path - Validate DID", func(tt *testing.T) {
		gock.New("https://demo.vc-sdk.com").
			Get("/.well-known/did.json").
			Reply(200).
			JSON(`{"didDocument": {"id": "did:web:demo.vc-sdk.com"}}`)
		defer gock.Off()
